function mutateString (mutator, val) 
{
    ...
    var negationInFile = true;
    while ((linesModified < numLinesToModify) && 
    (equalsInFile || zeroesInFile || stringsInFile || lessThanInFile || logicalAndInFile || negationInFile)) {
        // Swap "==" with "!="
        ...
        // Remove ! when it's negating some condition
        if (mutator.random().bool(0.2) && linesModified < numLinesToModify) {
            if (/!\w+/.test(val)) {
                var match = /!\w+/.exec(val);
                var matchedString = match[0];
                console.log(`Removing ! from ${matchedString}`);
                var removeNegation = matchedString.replace('!', '');
                val = val.replace(matchedString, removeNegation);
                linesModified++;
            } else {
                if (negationInFile) console.log('There is no negation of a variable in the file')
                negationInFile = false;
            }
        }
    }
    return val;
