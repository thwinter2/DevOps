...
var latency = setInterval( function () 
{
    ...
    got.post(`http://localhost:3090/preview`, {timeout: 5000, throwHttpErrors: false, json: JSON.parse(fs.readFileSync('/bakerx/survey.json'))}).then(function(res)
    {
        ...
        captureServer.latency.push((res.timings.end || res.timings.error || res.timings.abort) - res.timings.upload);

        // Capture the JSON response size. Since we're sending the same request
        // every time, we should expect to get back a response of the same size.
        // If the service started returning a 200 status code but was not returning
        // the expected response body, this should capture that.
        captureServer.responseLengths.push(res.body.length);
    }).catch( e => 
    {
        captureServer.requestResults.push(1);
        captureServer.latency.push(5000);
        captureServer.responseLengths.push(0);
    });
}, 1000);
...
var siege = child.spawn(`siege`, [`-t60s`, `-d1`, `-c5`, `--content-type`, `"application/json"`, `'${prod.TARGET}/preview`, `POST`, `</bakerx/survey.json'`], {shell:true});
...
siege.on('close', function(code) {
    ...
    siege = child.spawn(`siege`, [`-t60s`, `-d1`, `-c5`, `--content-type`, `"application/json"`, `'${prod.TARGET}/preview`, `POST`, `</bakerx/survey.json'`], {shell:true});
    ...
    siege.on('close', function(code) {
        ...
        //console.log(`\nGREEN RESPONSE SIZES: ${greenServer.responseLengths}, SIZE: ${greenServer.responseLengths.length}`);
        //console.log(`\nBLUE RESPONSE SIZES: ${blueServer.responseLengths}, SIZE: ${blueServer.responseLengths.length}`);
        passedMetrics += runStatisticalUTest(greenServer.responseLengths, blueServer.responseLengths, "RESPONSE SIZES");

        canaryScore = (passedMetrics / 5) * 100;
        console.log(`\nFINAL CANARY SCORE = (${passedMetrics} / 5) = ${canaryScore}%`);
        if (canaryScore < 40) {
